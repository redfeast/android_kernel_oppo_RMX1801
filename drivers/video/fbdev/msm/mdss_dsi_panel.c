...
#ifdef VENDOR_EDIT
//Shengjun.Gou@MultiMedia.Display.LCD.Stability, 2017/02/14,
...
/*
 * Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2017/05/18,
...
#ifdef VENDOR_EDIT
//Shengjun.Gou@MM.Display.LCD.Stability, 2017/02/18,
//add for lcd seed
enum
{
	SEED_CLOSED_MODE = 0,
...
//Shengjun.Gou@MultiMedia.Display.LCD.Stability, 2016/10/10,
...
#ifdef VENDOR_EDIT
//Shengjun.Gou@MultiMedia.Display.LCD.Stability, 2017/01/19,
...
//Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2017/01/20,
...
/* Gou shengjun@PSW.MM.Display.LCD.Stability, 2017/08/21 solve AOD flicker issue */
...
#ifdef VENDOR_EDIT
//Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2017/06/13
//add for 17011 17021 1024 level backlight
	int	BL_MSB = 0;
	int	BL_LSB = 0;
#endif /*VENDOR_EDIT*/
...
/*jie.hu@PSW.MM.Display.LCD.Stability,2018/2/24,delay 11 frame*/
/*jie.hu@PSW.MM.Display.LCD.Stability,2018/2/27,delay 1 frame,optimize performance and implement fast unblank*/
		mdelay(20);
		mutex_lock(&aod_lock);
...
/* Gou shengjun@PSW.MM.Display.LCD.Stability, 2017/08/21 solve AOD flicker issue */
	...
	memset(&cmdreq, 0, sizeof(cmdreq));
#ifdef VENDOR_EDIT
//Shengjun.Gou@PSW.MultiMedia.Display.LCD.Stability, 2017/06/13
//add for 1024 level backlight
	if((lcd_vendor == OPPO17011_SAMSUNG_SOFEG01_S_1080P_CMD_PANEL)
		|| (lcd_vendor == OPPO17021_SAMSUNG_SOFEG01_S_1080P_CMD_PANEL)
		|| (lcd_vendor == OPPO17081_SAMSUNG_AMS596W401_1080P_CMD_PANEL)
...
void set_hbm_level(struct mdss_panel_data *pdata, int hbm_level, bool hbm_to_aod)
{
	...
	if (lcd_vendor == OPPO18005_SAMSUNG_AMS641RW01_1080P_CMD_PANEL)
	{
		...
		if (hbm_mode == HBM_L0)
		{
			...
			pr_info("%s: HBM recovery 51: 0x%x, 0x%x.\n", __func__, new_oled_backlight[1], new_oled_backlight[2]);
		}
	...
	pr_info("%s: HBM 51: 0x%x, 0x%x; 53: 0x%x.\n", __func__, gl_ctrl_pdata->hbm_cmds.cmds[1].payload[1],
		gl_ctrl_pdata->hbm_cmds.cmds[1].payload[2],
		gl_ctrl_pdata->hbm_cmds.cmds[2].payload[1]);
...
#ifdef VENDOR_EDIT
//Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2017/02/18,
//add for lcd seed
...
#ifdef VENDOR_EDIT
//Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2017/02/28,
...
#ifndef VENDOR_EDIT
//Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2017/01/18,
...
//modify for 16103 panel power setting
			...
			*/
				...
				|| is_lcd(OPPO18321_DPT_NT36672A_1080_2340_VOD_PANEL))
			{
				...
				lm3697_bl_enable(1);
				//Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2017/03/02,
				//add for lcd power timing
				if (gpio_is_valid(ctrl_pdata->disp_en_gpio)) {
					rc = gpio_direction_output(ctrl_pdata->disp_en_gpio, 1);
					if (rc) {
...
#ifdef VENDOR_EDIT
//Ling.Guo@PSW.MM.Display, 2017/04/28
...
#ifdef VENDOR_EDIT
//Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2017/04/03,
//add for i2c backlight control
	...
	case BL_WLED:
#ifdef VENDOR_EDIT
/*Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2018/05/24,
 *add for wled debug
*/
...
//add for lcd cabc
	if(is_lcd(OPPO16103_JDI_R63452_1080P_CMD_PANEL)){
		...
		//Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2016/10/10,
		//add for lcd esd recovery power off when tp black gesture open
		if(!lcd_esd_status){
			lcd_esd_status = 1;
...
#ifdef VENDOR_EDIT
//Gou Shengjun@MultiMedia.Display.LCD.Stability, 2017/01/24,
//add for lcd debug
...
extern bool  fb_blank_sync_flag;
...
#ifdef VENDOR_EDIT
//Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2017/02/21,
...
//Shengjun.Gou@Multimedia.Driver.feature, 2017/03/17,
//add for HBM
		mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->hbm_cmds,
			"qcom,mdss-dsi-hbm-command", "qcom,mdss-dsi-off-command-state");
	}
	...
	int ndx)
{
	...
	struct mdss_panel_info *pinfo;
#ifdef VENDOR_EDIT
//Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2017/10/10
